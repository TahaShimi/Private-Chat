...
if ($_SESSION['id_link'] != null) {
    ...
    $packages->bindParam(':idu', $account->id_customer, PDO::PARAM_INT);
    $packages->bindParam(':IDL', $_SESSION['id_link'], PDO::PARAM_INT);
    $packages->bindParam(':lang', $_COOKIE["lang"], PDO::PARAM_STR);
    $packages->bindParam(':cur', $_SESSION["currency"], PDO::PARAM_STR);
    $packages->execute();
...
$packages = $conn->prepare("SELECT * FROM packages_price WHERE currency = (CASE WHEN (SELECT DISTINCT currency FROM packages_price pp where pp.currency=:cur AND pp.date_end IS null) IS NOT NULL THEN :cur ELSE (SELECT currency FROM accounts where id_account=:IDC) END) AND date_end IS null");
$packages->bindParam(':IDC', $id_account, PDO::PARAM_INT);
$packages->bindParam(':cur', $_SESSION["currency"], PDO::PARAM_STR);
$packages->execute();
$pkg = $packages->fetchAll(PDO::FETCH_ASSOC);
                ...
                <div class='pricing pricing-palden'>
                    <?php if ($pricings_rows > 0) {
                        foreach ($pricings as $pri) {
                            if ($pri['total_discount'] < 100 && isset($pkg[$pri["id_package"]])) {
                                if ($pri['total_discount'] > 0) {
                                    ...
                                    <ul class='pricing-feature-list'>
                                        <li class='pricing-feature'><span class='pricing-period'><del class='uppercase'>" . $pkg[$pri["id_package"]]['price'] . "</del></span>" . round(((100 - $pri['total_discount']) * ($pkg[$pri["id_package"]]['price'] / 100)), 2) . " " . $pkg[$pri["id_package"]]['currency'] . "</li>
                                    </ul>
                                ...
                                <ul class='pricing-feature-list'>
                                    <li class='pricing-feature'>" . round(((100 - $pri['total_discount']) * ($pkg[$pri["id_package"]]['price'] / 100)), 2) . " " . $pkg[$pri["id_package"]]['currency'] . "</li>
                                </ul>
                                <button class='pricing-action'><a href='javascript:void(0)' data-id='" . $pri["id_package"] . "' class='buy-offer' style='all: unset;'>" . ($trans["package_card"]["buy_now"]) . "</a></button>
                            </div>";
                                }
