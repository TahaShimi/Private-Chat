                        ...
                        <tbody>
                            <?php foreach ($contacts as $contact) { ?>
                                <tr id="<?= $contact['id_ticket'] ?>">
                                    ...
                                    else echo '<span class="badge badge-success badge-pill l">' . $trans['closed'] . '</span>' ?></td>
                                    <td><?= $contact['name'] ?></td>
                                    <td><?= $contact['date'] ?></td>
                ...
                if (data.length > 0) {
                    $.each(data, function() {
                        if (this.id_sender == sender) {
                            ...
                            $('#conv').append(`<li class="reverse"><div class="chat-content"><h5>${name}</h5><div class="box bg-light-info">${this.message}</div><div class="chat-time">` + moment.utc(this.date).local().format("MMMM Do YYYY, h:mm a") + `</div></div></li>`);
                        }
                    });
    ...
    $('#send_message').click(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('#total_contact').text(table.column(5).nodes().to$().find('.label-success').length);
                $('#overlay').hide();
            }
        });
    });
