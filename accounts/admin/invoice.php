...
$s2 = $conn->prepare("SELECT * from customers a where id_customer=:IDC");
$s2->bindParam(':IDC', $transcation["id_customer"], PDO::PARAM_INT);
$s2->execute();
$customer = $s2->fetchObject();

$s3 = $conn->prepare("SELECT * from accounts a where id_account=:IDA");
$s3->bindParam(':IDA', $id_account, PDO::PARAM_INT);
$s3->execute();
$company = $s3->fetchObject();
?>
             <div class="row">
                    <div class="col-md-12">
                        <div class="card card-body printableArea">
                            <h3><b><?php echo  ($trans["transactions_table"]["invoice"]) ?></b> <span class="pull-right">#<?= $transcation["id_transaction"]?></span></h3>
                            <hr>
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="pull-left">
                                    ...
                                    <div class="pull-right text-right">
                                        <address>
                                            ...
                                            <h4 class="font-bold"><?= $customer->firstname . ' ' . $customer->lastname ?>,</h4>
                                            <p class="text-muted m-l-30"><?= $customer->address ?>,
                                                <br/> <?= $countries[$customer->country] ?></p>
                                            <p class="m-t-30"><b><?php echo ($trans["invoice_date"])?> :</b> <i class="fa fa-calendar"></i> <?= $transcation["date_add"]?></p>
                                        </address>
                                    </div>
                                </div>
                                <div class="col-md-12">
                                ...
                                <div class="col-md-12">
                                    <div class="pull-right m-t-30 text-right">
                                       <!-- <p>Sub - Total amount: $13,848</p>
                                        <p>vat (10%) : $138 </p>-->
                                        <hr>
                                        <h3><b>Total :</b> <?= $transcation["final_price"]?> <sup><?= $transcation["currency"]?></h3>
                                    </div>
                                    <div class="clearfix"></div>
                                    <hr>
                                    <div class="text-right">
                                        <button id="print" class="btn btn-default btn-outline" type="button"> <span><i class="fa fa-print"></i> <?php echo  ($trans["print"]) ?></span> </button>
                                    </div>
                                </div>
                            </div>
                        </div>
