...
$directory = "delastro-fr";
include('../../init.php');
$items = explode("/", $_SERVER['PHP_SELF']);
$folder = $items[2];
$id_website = 1;

$stmt = $conn->prepare("SELECT a.*, b.`name`, b.`activity`, b.`seo_description`, b.`seo_keywords` FROM `websites_landing` a INNER JOIN `websites` b ON a.`id_website` = b.`id_website`  WHERE a.`id_website` = :fl");
...
$newcontent .= '<form name="registerForm" class="row register-form" novalidate="novalidate"><div class="col-md-12"><h4 class="h4-sm">Get started for FREE!</h4><p>Fill all fields so we can get some info about you. We’ll never send you spam</p></div><div id="input-name" class="col-md-12"><input class="form-control name" type="text" name="name" placeholder="Your Name"></div><div id="input-email" class="col-md-12"><input class="form-control email" type="text" name="email" placeholder="Your Email"></div><div id="input-phone" class="col-md-12"><input class="form-control phone" type="text" name="phone" placeholder="Your Phone Number"></div><div class="col-md-12"><button type="submit" class="btn btn-md btn-blue black-hover submit">Sign Up Now</button><p class="p-sm">By signing up, you accept our <a href="#">Terms</a> &amp; <a href="#">Privacy Policy</a></p></div><div class="register-form-msg text-center"><div class="sending-msg"><span class="loading"></span></div></div></form>';
}
